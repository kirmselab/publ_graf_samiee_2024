...
%           - event detection parameters is optimized for P10-12 (network
%           bursts showing a power increase in 4-15 Hz (theta-alpha) range)
%           - included SPW_thr_factor for SPW detection: threshold =
%           STD(LFP) * SPW_thr_factor
...
options.LFP_Ref =  [2084 2420]; % Reference recording for LFP, when electrode was still outside the brain
options.control = [3113 5385]; % control recording in s from start of file
% how to deal with different records?
options.C21 = [5522 13790]; % treatment recording in s from start of file
...
for n = 1:length(cond)
    ...
    bands.(field){5,1} = [6 20]; % custom data driven frequency band for HBOs (P4)
    bands.(field){6,1} = [25 80]; % custom data driven frequency band for HBOs (P4), even more specific    
end
drift_ext = 0.10; % extension in seconds to remove after noise peak detection; should not be smaller than 1/lfp.fs
nSec_base = 120; % window length in seconds for baseline calculation
SPW_thr_STD_factor = 10; % factor for determination of threshold (STD(LFP) * factor)
SPW_thr = -0.2; % LFP threshold for SPW detection
std_factor = 5; % factor for determination of threshold (median + factor * MAD)
...
for n = 1:size(lfp,2) % control, C21 and LFP reference
    ...
    driftidx{1,n} = [find(lfp{3,n}>0.15); find(lfp{3,n}<-0.15); find(abs(diff(lfp{3,n}))>0.3)]; % detect high amplitude noise peaks, meaning 0.2 mV per sample point, which is a really sharp increase
...
%%
a = lfp{3,1};
a_LPF = lowpass(a,100,metadata.lfp.fs,'ImpulseResponse','fir');
a_Hilbert = hilbert(a_LPF);
aAmp = abs(a_Hilbert);
aPhase = angle(a_Hilbert);
...
for n = 1:size(lfp,2) % control, C21 and LFP reference
    ...
    for i = 1:subsec.n{n} % for the number of subsections
        ...
        subsec.bp.(field){i} = bandpower(subsec.pxx.(field){i}, w, bands.control{4, 1} , 'psd'); % calculate bandpower of subsection, bands.control{4, 1} = 4-15 Hz
    end
end
clear o p 
...
params.tapers = [2 3]; % [TW K] TW is time-bandwidth product, K is number of tapers, K<=(TW*2)-1, smaller TW increases frequency resolution, [1 2] was good for P4, detecting SPWs, [2 3] was better for P11, [3 5] smears to much in frequency domain
...
d = c + a;
if a > driftperiods_all{1,n}(b,1)
    % do nothing, because d is larger than last value in driftperiods
else
...
for n = 1:size(lfp,2) % control, C21 and LFP reference
    field = string(cond{n});
    SPW_thr_STD(1,n) = std(lfp{3,n}) * -SPW_thr_STD_factor;
    [SPW.(field){1,1}, SPW.(field){1,2}] = peakfinder(lfp{3,n}, (max(lfp{3,n})-min(lfp{3,n}))/30, SPW_thr_STD(1,n), -1); % find LFP < SPW_thr
end
...
for n = 1:(size(lfp,2)-1) % control, C21 without LFP reference
    ...
    while min(b) < metadata.lfp.fs/2 % as long as the minimum distance between SPWs is smaller than 0.5 seconds
        [c,d] = min(b); % c is the minimum distance and d is index of that value, corresponding to the first SPW
