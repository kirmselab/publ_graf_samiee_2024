...
opt.FluorFile_name = 'sample_DETECTION_CRITERION_66ROIs.mat'; % name of the file containing the fluorescence data; e.g. 'mydata.txt'
...
opt.remove_drifts         = 1; % 1: remove slowly varying drifts, 0: skip the drift removal step
opt.remove_posDeflections = 0; % 1: apply large-impulse (deflection) removal step, 0:skip this step
opt.remove_negDeflections = 1; % 1: remove large short-lasting negative deflections, 0:skip this step
opt.demerging             = 1; % 1 --> apply the demerging step, 0 ---> skip it
opt.gen_FR_count          = 0; % 1 --> generate the estimated firing rate vector based on the reconstructed spike-count train, 0 --> skip it.
opt.gen_FR_count_dem      = 0; % 1 --> generate the estimated firing rate vector based on the reconstructed demerged spike-count train, 0 --> skip it.


%% 5: run UFARSA
[output_UFARSA,opt_out, ROIs_all] = run_UFARSA(opt); % KK: ROIs_all may slow down computation as results for all ROIs are stored in one variable
% KK: [output_UFARSA,opt_out, ROIs_all] = run_UFARSA(opt);
