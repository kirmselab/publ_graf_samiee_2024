...
%        opt.min_leading_amp --> []: leading threshold will be estimated, and be lower-bounded with an intrinsically determined minimum leading threshold (default)
...
%     fluors.afterSmoothing (fluors.afterSmoothing < 0) = 0; % *-* line
%     added 12.12.19 --- line commented for UFRASA_OGB1
...
if ZeroEvent_flag ~=1 % when there are non-zero amount of detected events
    ...
    [EventTimes_SR,vector_nSpikes_discrete_SR,idx_onset_Actu_B_SR] = de_merging();  % VRn: I adapted this for Matlab 2020
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
%%
    function [EventTimes_SR,vector_nSpikes_discrete_SR,idx_onset_Actu_B_SR] = de_merging() %VRn: I adapted this for Matlab 2020
...
elseif isempty(opt.min_leading_amp) % when opt.min_leading_amp = []; leading threshold will be estimated, and be lower-bounded with an intrinsically determined minimum leading threshold (default)
...
else % when opt.min_leading_amp = [scalar]; leading threshold will be estimated, and be lower-bounded with a user-determined minimum leading threshold
   
    min_thr_leading = opt.min_leading_amp; % the minimum-leading threshold (i.e. the lower-bound for leading threshold) determined by the user
