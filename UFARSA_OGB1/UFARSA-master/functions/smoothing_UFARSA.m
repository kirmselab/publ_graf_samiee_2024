...
% end
% *-* 25.11.2019: previous if ... end statement commented
Samples_indices = opt.nSamplesForSTD;
% *-* 25.11.2019: previous line added
...
if opt.test_smoothing ==1 && isempty(opt.smoothing_param) 
    ...
    if smoothing_param<=5 && opt.test_smallS==1 && opt.sim_flag==0, cond4 = 1; else cond4 = 0; end % when computed smoothing parameter is less than 1
    
    if smoothing_param>=opt.max_smoothing_param % *-* 30.01.2020: lines were added
        cond5 = 1; % *-* 30.01.2020: lines were added
    else % *-* 30.01.2020: lines were added
        cond5 = 0; % *-* 30.01.2020: lines were added
    end % *-* 30.01.2020: lines were added
    
    if cond1 || cond2 || cond3 || cond4 || cond5
        
        if cond5 % *-* 30.01.2020: lines were added
            smoothing_param = opt.max_smoothing_param; % *-* 30.01.2020: lines were added
        else % *-* 30.01.2020: lines were added
